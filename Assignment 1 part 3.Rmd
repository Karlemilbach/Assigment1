...
library(stringr);library(tidyverse);library(plyr);library(dplyr);library(lme4);library(modelr);library(caret)
...
for (i in 1:352)
  {
  DataC1=data.train[data.train==i,]
  DataCV=data.train[data.train!=i,]
  M1 = lmer(CHI_MLU ~ VISIT * Diagnosis + (1+VISIT|ID), data=DataCV)
  P1 = predict(M1,DataC1, allow.new.levels=T)
  names(P1)=NULL
  P1
  PTOT = c(PTOT,P1)
}
