...
```{r}
colnames(LU_train)[1]="ID"
colnames(token_train)[1]="ID"
colnames(demo_train)[1]="ID"
...
```{r}
#Extract only digits and the save to the variable
regexp="[[:digit:]]+"
LU_train$VISIT=str_extract_all(LU_train$VISIT,regexp)
token_train$VISIT =str_extract(token_train$VISIT,regexp)

#or use \\d : demo_train$VISIT=str_extract(demo_train$VISIT,"\\d")
```
...
```{r}
#rename columns to something we remember what is
colnames(demo_train)[24]="nonVerbalIQ"
colnames(demo_train)[35]="VerbalIQ"
...
demo_train2=select(demo_train,1,2,3,4,7,9,14,24,35)
LU_train2=select(LU_train,1,2,3,4,8,9)
token_train2=select(token_train,1,2,3,4,6,7)

#merge the different dataframes
mergesemifinal=merge(demo_train2,LU_train2)
final=merge(mergesemifinal,token_train2)
```
...
```{r}
#Give each child a unique number
...
final$Gender=revalue(final$Gender,c("M"="1", "F"="2"))
...
```{r}
write.csv(final,file="Assignment1data")
```
...
```{r}
#1)
#More than 2.7 morphemes at least once across the visits
unique(select(filter(final, CHI_MLU>2.7),ID))


#2)
#Less than 1.5 morphemes
final$ID[final$VISIT==1 & final$CHI_MLU < 1.5]

#3)
#dataframe with children who have not completed all trials
df =final[final$ID %in% names(table(final$ID))[table(final$ID)<6],]
...
```{r}
#1)
arrange(select(final[which(final$VISIT=="6"),],1,17), desc(tokens_CHI))
#Kid number 55

#visit6=final[which(final$VISIT=="6"),]
#visit6=select(visit6,1,17)
#arrange(visit6,desc(tokens_CHI))

#2)
arrange(select(final[which(final$VISIT=="1"),],1,17),tokens_CHI)
#Kid number 57

#leastwords=final[which(final$VISIT=="1"),]
#leastwords=select(leastwords,1,17)
#arrange(leastwords,tokens_CHI)
...
#1)
#Select columns
ASD=select(final,1,7,12,17)

#Select only kids with ASD
ASD=ASD[ASD$Diagnosis=="ASD",]

#2)
#The variable is selected only once
...
```{r}
#1)
#Add the grand mean to a new column
final$grand_mean <- mean(final$tokens_CHI)

#2)
#Ad a column with each child's mean 
meanwords=group_by(final, ID) %>% summarize(mean_tokens_CHI = mean(tokens_CHI))
final1=merge(final,meanwords,by="ID")

#3)
#The mean is a generalized measure. Maybe the child scored very differently in each visit. More data is more information that might be relevant
